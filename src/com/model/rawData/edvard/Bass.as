package com.model.rawData.edvard
{
public class Bass
{
public static var data:XML =  
<bass type="bass" thumbNail='bass_flash.jpg' spriteSheet='bassSheet.png'>
<bubble x="-155" y="22">
</bubble>
	<eyes swfSheet="bassBlink.png" x="0" y="0" width="250" height="350" >
		<blink start="0" end="5" />
	</eyes>	
<components recordButtonX="320" recordButtonY="580">
		
		<bass image="BASS_ICON_1_IDLE.png" playImage="BASS_ICON_1_IDLE.png" x='0' y='0'  noteX='15' noteY='5'>
			<tap noteId="1" />
		</bass>
		<bass image="BASS_ICON_2_IDLE.png" playImage="BASS_ICON_2_IDLE.png" x='140' y='0'  noteX='15' noteY='5'>
			<tap noteId="2" />
		</bass>
		<bass image="BASS_ICON_3_IDLE.png" playImage="BASS_ICON_3_IDLE.png" x='280' y='0'  noteX='15' noteY='5'>
			<tap noteId="3" />
		</bass>
		
		<bass image="BASS_ICON_4_IDLE.png" playImage="BASS_ICON_4_IDLE.png" x='420' y='0'  noteX='15' noteY='5'>
			<tap noteId="4" />
		</bass>
		<bass image="BASS_ICON_6_IDLE.png" playImage="BASS_ICON_6_IDLE.png" x='560' y='0'  noteX='15' noteY='5'>
			<tap noteId="6" />
		</bass>
		
	</components>
	<studio>
	<background image="backgr_solo.jpg" />
	<spotLight image="bass_spot_lights.png" />
	</studio>
<octaves>		
	<notes>
	<note id="1"   soundFile="edvard/notes/bass/B.mp3" animationIndex='1' value="1"  />
	<note id="2"   soundFile="edvard/notes/bass/D.mp3" animationIndex='2' value="2"  />
<note id="3"   soundFile="edvard/notes/bass/F+1.mp3" animationIndex='3' value="3"  />
			
<note id="4"   soundFile="edvard/notes/bass/C+1.mp3" animationIndex='4' value="4"  />
	<note id="5"   soundFile="edvard/notes/bass/E.mp3" animationIndex='5' value="5"  />
<note id="6"   soundFile="edvard/notes/bass/F+1OH.mp3" animationIndex='6' value="6"  />
<note id="7"   soundFile="edvard/notes/bass/F+1OH.mp3" animationIndex='7' value="7"  />
<note id="8"   soundFile="edvard/notes/bass/F+1OH.mp3" animationIndex='8' value="8"  />
	</notes>
</octaves>	
	<gestures>
	</gestures>
	<sequances>
	<sequance id="2">
		<note id="1" location="1" soundLength="2"/>
		<note id="1" location="13" soundLength="1"/>
		<note id="1" location="17" soundLength="4"/>
		<note id="1" location="33" soundLength="2"/>
		<note id="1" location="45" soundLength="1"/>
		<note id="1" location="49" soundLength="4"/>
		<note id="1" location="65" soundLength="2"/>
		<note id="1" location="77" soundLength="1"/>
		<note id="1" location="81" soundLength="4"/>
		<note id="1" location="97" soundLength="2"/>
		<note id="1" location="109" soundLength="1"/>
		<note id="1" location="113" soundLength="4"/>
		
	</sequance>
	<sequance id="3">
		<note id="1" location="1" soundLength="2"/>
		<note id="1" location="13" soundLength="1"/>
		<note id="1" location="17" soundLength="4"/>
		<note id="1" location="33" soundLength="2"/>
		<note id="1" location="45" soundLength="1"/>
		<note id="1" location="49" soundLength="4"/>
		<note id="2" location="65" soundLength="2"/>
		<note id="2" location="77" soundLength="1"/>
		<note id="2" location="81" soundLength="4"/>
		<note id="2" location="97" soundLength="4"/>
		
	</sequance>
<sequance id="4">
		<note id="3" location="1" soundLength="2"/>
		<note id="3" location="13" soundLength="1"/>
		<note id="3" location="17" soundLength="4"/>
		<note id="3" location="33" soundLength="2"/>
		<note id="3" location="45" soundLength="1"/>
		<note id="3" location="49" soundLength="4"/>
		<note id="2" location="65" soundLength="2"/>
		<note id="2" location="77" soundLength="1"/>
		<note id="2" location="81" soundLength="4"/>
		<note id="3" location="97" soundLength="2"/>
		<note id="3" location="109" soundLength="1"/>
		<note id="3" location="113" soundLength="4"/>
		
	</sequance>
<sequance id="5">
		<note id="3" location="1" soundLength="2"/>
		<note id="3" location="13" soundLength="1"/>
		<note id="3" location="17" soundLength="4"/>
		<note id="3" location="33" soundLength="2"/>
		<note id="3" location="45" soundLength="1"/>
		<note id="3" location="49" soundLength="4"/>
		<note id="2" location="65" soundLength="2"/>
		<note id="2" location="77" soundLength="1"/>
		<note id="2" location="81" soundLength="4"/>
		<note id="3" location="97" soundLength="4"/>
		
	</sequance>
<sequance id="6">
<note id="1" location="1" soundLength="2"/>
		<note id="1" location="13" soundLength="1"/>
		<note id="1" location="17" soundLength="4"/>
		<note id="1" location="33" soundLength="2"/>
		<note id="1" location="45" soundLength="1"/>
		<note id="1" location="49" soundLength="4"/>
		<note id="1" location="65" soundLength="2"/>
		<note id="1" location="77" soundLength="1"/>
		<note id="1" location="81" soundLength="4"/>
		<note id="2" location="97" soundLength="2"/>
		<note id="2" location="109" soundLength="1"/>
		<note id="2" location="113" soundLength="4"/>

	</sequance>
			
<sequance id="7">
		<note id="3" location="1" soundLength="2"/>
		<note id="3" location="13" soundLength="1"/>
		<note id="3" location="17" soundLength="4"/>
		<note id="2" location="33" soundLength="2"/>
		<note id="3" location="45" soundLength="1"/>
		<note id="3" location="49" soundLength="4"/>
		<note id="3" location="65" soundLength="2"/>
		<note id="3" location="77" soundLength="1"/>
		<note id="3" location="81" soundLength="4"/>
		<note id="2" location="97" soundLength="2"/>
		<note id="3" location="109" soundLength="1"/>
		
	</sequance>
			
<sequance id="8">
<note id="1" location="1" soundLength="2"/>
		<note id="1" location="13" soundLength="1"/>
		<note id="1" location="17" soundLength="4"/>
		<note id="1" location="33" soundLength="2"/>
		<note id="1" location="45" soundLength="1"/>
		<note id="1" location="49" soundLength="4"/>
		
		<note id="6" location="65" soundLength="2"/>
		<note id="5" location="73" soundLength="2"/>
		<note id="2" location="81" soundLength="2"/>
		<note id="4" location="89" soundLength="2"/>
		<note id="1" location="97" soundLength="2"/>
		<note id="3" location="105" soundLength="2"/>
		
	</sequance>
	<sequance id="99">
	<subSequance id="6" location="0" />
	<subSequance id="7" location="128" />
	<subSequance id="7" location="256" />
	<subSequance id="8" location="384" />
	</sequance>
	</sequances>
</bass>
		public function Bass()
		{
		}
	}
}